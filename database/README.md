...
3. **Run the Migrations**
   ...
   | 21 | `021_allow_published_recipes_in_cookbooks.sql` | Drops and recreates the `cookbook_recipes` INSERT policy so the **recipe** half is own-or-published, mirroring `012`; the cookbook-ownership half is unchanged. **One policy only** — no schema, index, grant, function or cascade change, and nothing touched on `recipes` (REW-100) |
...
A per-user named collection of recipes ("cookbooks") — the owner's own recipes at any status, plus, as of REW-100 (migration `021`), any other user's **published** recipes. Modeled directly on the `recipes` table pattern. Private by default; REW-19 adds an opt-in Public state (see `is_public` below).
...
Junction table linking cookbooks to recipes (many-to-many) — a single recipe can belong to any number of cookbooks, and a cookbook can hold any number of recipes. Since REW-100 (migration `021`) those recipes need not all belong to the cookbook's owner: the owner's own recipes at any status, plus any other user's **published** recipes, are all eligible. Only the cookbook's owner can add, see or remove its membership rows:
...
**INSERT visibility rule:** the INSERT RLS policy allows adding a recipe that is **either the caller's own recipe (any status) or any other user's *published* recipe** — not owner-only. This mirrors the visibility rule already used by `recipe_likes`, and reflects that "Add to Meal Plan" appears on `/browse`, `/search`, and `/recipes/liked`, which show other users' published recipes. When `012` shipped this was the **key difference** from `cookbook_recipes`, which was owner-only; that difference is gone as of REW-100 (migration `021`), which widened the `cookbook_recipes` INSERT policy to this same shape. `012` was the precedent `021` copied.
...
- INSERT requires **both** cookbook ownership **and** a recipe-visibility check. **As of REW-100 (migration `021`) the recipe half is own-or-published, not owner-only:** `(recipes.user_id = auth.uid() OR recipes.status = 'published')` — a user can add their own recipe at any status, or any other user's **published** recipe, into a cookbook **they own**. Another user's draft recipe is still rejected by Postgres even if application code were buggy, and the cookbook-ownership `EXISTS` is unchanged, so a user still cannot insert into a cookbook belonging to somebody else. This is now the same rule `meal_plan_recipes` has had since `012`; migration `010`'s header, which documents own-recipes-only as a deliberate database-layer guarantee, is **historical** from `021` onward (`010` itself was not edited — it has already been applied). Note the policy name was deliberately kept (`"Users can insert own cookbook recipes"`), exactly as `012`'s analogous policy is named "own" while admitting published rows
...
- Cookbooks have no publish/draft state and can contain a mix of the owner's draft and published recipes — cookbook membership is independent of a recipe's `status`. Both a recipe's own draft/published lifecycle and its cookbook membership can change independently of each other. **As of REW-100 (migration `021`) a cookbook can also contain other users' published recipes**, so it is a genuinely mixed-ownership collection, exactly like a meal plan.
...
- **REW-86 requires no migration.** The card-level Private/Public toggle (`POST /recipes/:id/visibility`) writes `recipes.status` on a row the caller owns, which migration `001`'s owner UPDATE policy already permits, and the handler additionally filters on `user_id` alongside RLS. The `+ Cookbook` card action inserts into `cookbook_recipes` for an owned cookbook and an owned recipe — the narrowest case migration `010`'s INSERT policy already allows, so no policy widening was needed (REW-59's `ALTER POLICY` for *other users'* Public recipes is a separate decision and is not part of REW-86). Favorite, meal-plan add, Edit, and Delete reuse existing policies unchanged. Nothing in this change alters a schema, a grant, an index, or any cascade behavior. **Live RLS verification was not performed — QA was not run for REW-86 and the branch is not merged.**
- **REW-88 requires no migration.** The standardized cookbook card widened one existing read — `GET /cookbooks/:id` now also selects `recipes.user_id` and `recipes.original_author` and embeds `recipe_categories(categories(...))` / `recipe_tags(tags(...))`, and stamps favorite state from one batched `recipe_likes` query. Every one of those reads is already permitted by applied policies: migration `001` grants SELECT on own rows and on any `status = 'published'` row, all columns; migrations `005`/`006` grant owner SELECT on the junction tables, which is what makes category and tag chips readable on the owner's **Private** recipes; migration `013` adds published-recipe SELECT for `anon`/`authenticated` plus the matching `categories`/`tags` lookups. Both halves of the junction access are needed on this page and both already exist. `recipe_likes` is read on the request-scoped (RLS-scoped) client with an explicit `user_id` filter on top, and is covered by the table's `(user_id, recipe_id)` primary key. Nothing in this change alters a schema, a policy, a grant, an index, or any cascade behavior, and no write path was touched. Note that `recipes.user_id` is fetched **only** to decide whether Edit/Delete are drawn — it is never rendered into the page and is never the authorization check. **No live verification was performed — QA was deliberately skipped for REW-88 and the branch is unmerged.** Specifically outstanding: the three-level PostgREST embed (`cookbook_recipes` → `recipes` → `recipe_categories` → `categories`) is the first of its kind in this repo and has not been run against a real database; its helper returns `[]` on error, which renders as an empty cookbook with no flash.
- **REW-89 requires no migration.** The standardized meal plan card widened one existing read — `GET /meal-plans/:id` now also selects `recipes.user_id` and `recipes.original_author` and embeds `recipe_categories(categories(...))` / `recipe_tags(tags(...))`, and stamps favorite state from one batched `recipe_likes` query. The reads are exactly the ones REW-88 already added on the cookbook page, so the same applied policies cover them: `001` (own rows plus any published row, all columns), `018` (`original_author`), `005`/`006` (owner SELECT on the junction tables, which makes chips readable on the owner's **Private** recipes), and `013` (published-recipe SELECT for `anon`/`authenticated` plus the matching `categories`/`tags` lookups). `recipe_likes` is read on the request-scoped (RLS-scoped) client with an explicit `user_id` filter on top. The grocery-list query `getMealPlanRecipeIngredients()` was deliberately left narrow and untouched, so its REW-26 skipped-row counting behavior is unchanged. Nothing alters a schema, policy, grant, index or cascade behavior, and no write path was touched — **020 remains the highest applied migration.** One thing is genuinely different from every earlier card surface: because migration `012`'s `meal_plan_recipes` INSERT policy admits your own recipes **or** anyone's published recipe, a meal plan can really contain another user's recipe, so `recipes.user_id` — fetched only to decide whether Edit/Delete are drawn, never rendered, never the authorization check — is a **live** production gate here rather than future-proofing. **No live verification was performed: the QA stage was excluded from the REW-89 run and the change is uncommitted.** Still outstanding from REW-88 and now also applicable here: the three-level PostgREST embed has not been run against a real database, and its helper returns `[]` on error, which renders as an empty plan with no flash.
- **REW-100 adds migration `021_allow_published_recipes_in_cookbooks.sql`** — the first change to `cookbook_recipes` enforcement since `019`, and a **knowing relaxation of a guarantee `010` documents as deliberate.** It drops and recreates exactly one policy, the `cookbook_recipes` INSERT policy, keeping its name and keeping the cookbook-ownership `EXISTS` byte-for-byte; only the recipe `EXISTS` widens to `(recipes.user_id = auth.uid() OR recipes.status = 'published')`, copied from `012`. Nothing else moves: `010`'s SELECT and DELETE policies stay cookbook-owner-scoped (so a recipe's author still cannot read or remove anything inside someone else's cookbook), both REW-19 public SELECT policies from `019` are untouched, and no policy, grant or column on `recipes` is altered — `001`'s own-or-published SELECT policy is **not** widened. No schema, index, grant, function or cascade change. Application-side, `handleCookbookRecipeAdd` in `src/routes/cookbookApiRoutes.js` now selects `id, user_id, status` filtered on `id` alone and enforces own-or-published in JavaScript, deliberately independent of RLS, with every reject path returning the same generic `404 Recipe not found` and no write. The two form-based cookbook add routes in `cookbookRoutes.js` stay own-recipes-only on purpose (their UI entry points are owner-gated, and narrower-than-RLS is always safe), so their ownership enforcement is now application-layer only.
  - **Known, accepted edge case (Private-after-add).** Once a cookbook can hold another user's recipe, that recipe's author can later flip it to Private. The `cookbook_recipes` row survives — the same gap already documented above for `recipe_likes` and `meal_plan_recipes`. The recipe then silently disappears from `/cookbooks/:id`, because `getCookbookRecipes` drops any junction row whose embedded `recipes` came back null under RLS, but `getCookbookRecipeCounts` counts membership rows with no recipe-visibility check — so `/cookbooks` can advertise "3 recipes" for a cookbook whose detail page renders 2. Accepted rather than fixed here, matching how the analogous `recipe_likes` gap was handled; tracked as [REW-109](https://wanderingnerds.atlassian.net/browse/REW-109). No privacy consequence: the hidden recipe's title, content and status are never rendered to a non-owner, only an inflated integer.
  - **REW-100 deployment — the order matters.** Apply `021` manually in the Supabase SQL editor after `020`, **before or together with the application deploy, never after it.** Deploying the application first does not just delay the fix, it degrades the current behavior: `handleCookbookRecipeAdd` now accepts another user's published recipe and hands it straight to the `cookbook_recipes` upsert, so if the app ships while `021` is unapplied, `010`'s still-in-force `WITH CHECK` rejects the insert and the user gets `500 {"error":"Failed to add recipe to cookbook"}` where today they get a clean `404 {"error":"Recipe not found"}`. Nothing is written in either case, so there is no data-integrity risk — only a worse failure than the bug being fixed. The same applies in reverse to the rollback below: revert the application alongside the policy. No backfill, no environment variable, no package, and no Vercel configuration change.
  - **Not applied anywhere, and QA was not run.** Migration `021` ships with automated coverage only (`src/routes/cookbookApiRoutes.test.js` asserts the SQL's content; `src/views/recipeCard.test.js` covers the card). **020 remains the highest applied migration.** As of 2026-09-23 the change is committed on branch `REW-100-add-published-recipe-to-cookbook` and nowhere else — not pushed, no pull request, not merged; read that as a point-in-time statement and re-check the branch rather than trusting this line later. Every live check in the ticket's acceptance criteria — the direct PostgREST insert matrix under RLS (published accepted, draft rejected, non-owned cookbook rejected), the author's inability to read or delete from someone else's cookbook, and the `/c/:id` render-then-disappear behavior when the author goes Private — remains unverified.
- **REW-101 requires no migration.** Adding route-level `csrfProtection` to `POST /recipes/:id/delete` is a request-authenticity check that runs before the delete handler; it is not an authorization change. The handler's `.eq("user_id", req.user.id)` ownership filter, the owner-only RLS DELETE policy on `recipes`, and the `ON DELETE CASCADE` behavior on `cookbook_recipes.recipe_id` / `meal_plan_recipes.recipe_id` (documented above) are all untouched, and no schema, policy, grant or index changed. **020 remains the highest applied migration.** Branch-only: reviewed, QA skipped, not pushed or merged.
...
To revert **only** REW-100 (migration `021`), putting cookbook membership back to own-recipes-only at the database layer and leaving cookbooks and their contents intact:
...
This restores `010`'s `WITH CHECK` verbatim and deletes nothing. Any non-owned membership rows inserted while `021` was in force **remain** in place, remain readable by the cookbook's owner, and remain removable by them — only *new* non-owned inserts stop being possible. The application must be reverted alongside it, or `handleCookbookRecipeAdd` will accept another user's published recipe and then fail on the insert under RLS.
