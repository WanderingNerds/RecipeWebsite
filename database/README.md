...
- **REW-101 requires no migration.** Adding route-level `csrfProtection` to `POST /recipes/:id/delete` is a request-authenticity check that runs before the delete handler; it is not an authorization change. The handler's `.eq("user_id", req.user.id)` ownership filter, the owner-only RLS DELETE policy on `recipes`, and the `ON DELETE CASCADE` behavior on `cookbook_recipes.recipe_id` / `meal_plan_recipes.recipe_id` (documented above) are all untouched, and no schema, policy, grant or index changed. **020 remains the highest applied migration.** Merged to `main` in PR #62 (commit `f8673b1`); QA was skipped on that run, so the manual delete pass is still outstanding.
- **REW-102 requires no migration.** The change is middleware wiring only: route-level `csrfProtection` added to `POST /meal-plans/:id/recipes/:recipeId/remove`, `POST /cookbooks/:id/recipes/:recipeId`, `POST /api/meal-plans/:id/recipes/:recipeId` and `POST /api/likes/:recipeId`. CSRF is a request-*authenticity* check; it is not an authorization change. The owner-only RLS policies on `cookbook_recipes`, `meal_plan_recipes` and `recipe_likes` (migrations `008`, `010`, `012`, plus `019`/`020`'s public-share SELECT policies) remain the authorization layer and are untouched, as are every handler's `.eq("user_id", …)` filter, `UUID_PATTERN` guard and the `ON DELETE CASCADE` behavior documented above. No table, column, index, constraint, grant or policy changed. **020 remains the highest applied migration.** Reviewed and approved; **the QA stage was skipped**, so no live database or browser verification was performed — nothing about this change requires any, since no query was altered. Deployment is a standard deploy with no migration step, no new environment variable and no new dependency.
