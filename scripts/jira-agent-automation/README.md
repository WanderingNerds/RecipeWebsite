...
- **Unattended execution runs with `--permission-mode auto`.** There's no human present to answer Claude Code's normal permission prompts, so a safety classifier reviews actions instead of you (this is the currently documented way to run `-p` unattended - see [Run Claude Code programmatically](https://code.claude.com/docs/en/headless) - and is meaningfully safer than fully disabling permission checks, though not a perfect substitute for a human reviewing every action). Combined with the fact that Jira ticket content is untrusted external text, there's still a real prompt-injection surface: a malicious or compromised ticket could try to get the Developer agent (which has Bash access) to do something other than implement the ticket. The pipeline prompt explicitly tells every agent to treat Jira content as data, not instructions, each agent's tool access is already scoped tightly in `.claude/agents/*.md` (Planner and Documentation can't touch code at all), and the classifier is a further backstop - but no combination of these is a perfect defense. Mitigations worth considering: run this under a Windows account with no more filesystem/network access than the repo needs, glance at `logs\jira-agent-runs\` periodically, and don't assign this Jira project to people/integrations you don't trust.
- **Every run writes a live-updating log** to `logs\jira-agent-runs\<timestamp>.log`. It uses `claude`'s `stream-json` output, reformatted into short readable lines (session start, each tool call, assistant text, the final result) as they happen - not buffered until the run finishes - so you can `Get-Content -Wait` the newest log file and watch a run progress in real time instead of wondering if it's stuck.
