...
const { handleMealPlanVisibilityUpdate } = await import("./mealPlanRoutes.js");
 ...
 * live Supabase. Mirrors cookbookVisibilityRoutes.test.js.
 */
function fakeClient({ data = { id: MEAL_PLAN_ID }, error = null } = {}) {
  const calls = [];
  const tables = [];
  const query = {
...
test("visibility flips both directions from the private|public form convention", async () => {
  for (const [visibility, expected] of [["public", true], ["private", false]]) {
    ...
    await handleMealPlanVisibilityUpdate(request({ body: { visibility } }), res, client);

    assert.deepEqual(client.tables, ["meal_plans"]);
    assert.deepEqual(client.calls[0], ["update", { is_public: expected }]);
    assert.equal(res.redirectPath, `/meal-plans/${MEAL_PLAN_ID}`);
  }
});

test("toggling visibility writes nothing but is_public", async () => {
  // AC13: flipping the share switch must never touch a recipe's status, plan
  // membership, or the plan's date range.
  const client = fakeClient();
  await handleMealPlanVisibilityUpdate(
    request({ body: { visibility: "public", title: "Hacked", startDate: "2020-01-01" } }),
    responseRecorder(),
    client
  );

  assert.deepEqual(client.tables, ["meal_plans"]);
  assert.deepEqual(Object.keys(client.calls[0][1]), ["is_public"]);
});
...
test("every update is scoped by both id and user_id, not by RLS alone", async () => {
  for (const visibility of ["public", "private"]) {
    ...
    assert.deepEqual(
      ...
      [["eq", "id", MEAL_PLAN_ID], ["eq", "user_id", "owner-1"]]
    );
  }
});
...
test("an unrecognized or absent visibility value fails closed to Private", async () => {
  for (const visibility of [
    ...
    assert.deepEqual(
      ...
      `${String(visibility)} must not make a meal plan Public`
    );
  }
});

test("a malformed meal plan id never reaches the database", async () => {
  for (const id of ["not-a-uuid", "", "../../etc/passwd", `${MEAL_PLAN_ID}'; DROP TABLE meal_plans;--`]) {
    const client = fakeClient();
    const res = responseRecorder();
    const flashes = [];
    await handleMealPlanVisibilityUpdate(
    ...
    assert.equal(res.redirectPath, "/meal-plans");
...
test("a not-found row is not logged as a database failure, but still reads identically", async () => {
  ...
  // tell a meal plan they don't own from one that doesn't exist.
...
test("the route is mounted behind requireAuth and the shared meal plan rate limiter", async () => {
  const { default: router } = await import("./mealPlanRoutes.js");
  const layer = router.stack.find(
    (entry) => entry.route?.path === "/:id/visibility" && entry.route?.methods?.post
  );

  assert.ok(layer, "POST /meal-plans/:id/visibility should be registered");
  ...
  assert.equal(
    ...
    3,
    "chain should be requireAuth + mealPlanLimiter + handler"
  );
});
