/**
 * REW-105: route-level CSRF protection on POST /meal-plans/:id/delete.
 *
 ...
 * reaches the handler and deletes the victim's meal plan. These tests pin the
 ...
 * per-user rate limiter (mealPlanLimiter). csrfProtection must sit AHEAD of
...
const DELETE_PATH = `/meal-plans/${MEAL_PLAN_ID}/delete`;
...
function deleteRouteHandles() {
  ...
  assert.ok(layer, "POST /meal-plans/:id/delete should be registered");
...
test("POST /meal-plans/:id/delete is mounted as requireAuth -> csrfProtection -> mealPlanLimiter -> handler", () => {
  const layer = findDeleteLayer();
  assert.ok(layer, "POST /meal-plans/:id/delete should be registered");
...
function buildHarness({ withRouteCsrf = true } = {}) {
  ...
  app.post("/meal-plans/:id/delete", ...chain);
...
test("a multipart POST is rejected even when it carries a valid _csrf part (no Multer stage on this route)", async (t) => {
  ...
  await withServer(app, async (base) => {
    ...
    // views/meal-plans/view.ejs posts urlencoded, so this is correct - and it
