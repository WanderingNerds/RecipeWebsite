...
- CSRF is enforced globally for unsafe non-multipart requests, but the global wrapper exempts *every* multipart POST rather than only the Multer routes that need it. Newer state-changing routes therefore re-apply `csrfProtection` explicitly at the route level — `POST /recipes/:id/clone`, `POST /recipes/:id/visibility`, `POST /recipes/:id/delete` (REW-101, on branch — reviewed, not QA-verified, not pushed or merged), and every `/api/cookbooks*` mutation. On all of these except `/:id/clone` the CSRF check runs ahead of the route's rate limiter, so a forged request cannot burn the victim's quota (the delete route has no per-route limiter); the clone route runs `addRecipeLimiter` first, an ordering nit tracked under REW-99. The delete route was the one destructive route with no route-level check at all — a forged cross-site multipart POST reached its handler — and REW-101's audit of every mutating route is recorded in the [API docs](docs/api/README.md#route-level-csrf-audit-rew-101); the two remaining destructive routes with the same gap (`POST /cookbooks/:id/delete`, `POST /meal-plans/:id/delete`) are tracked as REW-105. The central fix is tracked as [REW-99](https://wanderingnerds.atlassian.net/browse/REW-99).
