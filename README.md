...
│   │   ├── cookbookApiRoutes.js # Cookbook JSON API backing the "+ Cookbook" card modal (REW-86); own-or-published recipe rule on add (REW-100)
...
│   ├── partials/recipe-summary-card.ejs # Shared recipe card; one `surface` local drives Browse, My Recipes, My Favorites, Cookbook and Meal Plan (REW-59, REW-86, REW-87, REW-88, REW-89, REW-100)
...
### Cookbooks (REW-62)
- **Create, Rename, and Delete Cookbooks**: Authenticated users can organize recipes into named, private collections ("cookbooks") from a dedicated "My Cookbooks" area (linked from the navbar). A cookbook holds any number of recipes, and the same recipe can belong to multiple cookbooks at once. When REW-62 shipped a cookbook could hold only its owner's own recipes; as of REW-100 it can also hold any other user's Public recipe — see "Adding Another User's Public Recipe to a Cookbook" below.
...
### Adding Another User's Public Recipe to a Cookbook (REW-100)

*Implemented and code-reviewed (approved over two rounds, no blocking issues). **The QA stage was deliberately excluded from this run — QA did not run.** As of 2026-09-23 the change is committed on branch `REW-100-add-published-recipe-to-cookbook` only: not pushed, no pull request, not merged. Treat that as a point-in-time statement and re-check the branch's merge state rather than trusting this line on a later date.*

`+ Cookbook` appears on every recipe card that can use it, including cards for recipes you don't own on **My Favorites**. Clicking it for someone else's recipe used to fail with a generic "Recipe not found" error toast. It now works for any other user's **Public** recipe.

- **What you can add to a cookbook**: any of your own recipes, Private or Public, plus any other user's **Public** recipe. Another user's **Private** recipe is still refused, and the refusal is deliberately indistinguishable from "that recipe doesn't exist" — no message reveals that the recipe exists but is Private.
- **You can still only add to a cookbook you own.** That half of the rule did not change, at either the application or the database layer.
- **A cookbook is now a mixed-ownership collection**, exactly like a meal plan already was. On `/cookbooks/:id`, cards for recipes you don't own keep the title, heart, chips, metadata, `+ Meal Plan`, Share and Remove, and show no Edit, no Delete, and no Private/Public pill.
- **Removing someone else's recipe from your cookbook never touches the recipe itself** — same behaviour as your own recipes. Equally, a recipe's author gains nothing by someone adding their recipe: they cannot read, or remove anything from, a cookbook they don't own.
- **Clearer copy on disabled hearts.** A disabled heart on a recipe you don't own now reads "Favorites are unavailable for this recipe" instead of "Make this recipe Public to add it to favorites", which was an instruction only the owner could act on and a claim about someone else's recipe's visibility. The owner still sees the original, actionable label.
- **The two form-based add paths deliberately stay own-recipes-only**: the "Save to Cookbook(s)" widget on a recipe's own page, and the bulk picker at `/cookbooks/:id/add-recipes`. Both are reached only from owner-gated UI, so keeping them narrow is intentional, not an oversight.
- **Known, accepted gap**: if an author makes a recipe Private after you've added it, it disappears from your cookbook's page but is still counted on `/cookbooks`, so the list can say "3 recipes" where the detail page shows 2. No content is exposed — only the number is wrong. Tracked as [REW-109](https://wanderingnerds.atlassian.net/browse/REW-109).
- **Requires migration `021_allow_published_recipes_in_cookbooks.sql`, and the order matters.** Apply it in Supabase **before or together with** the application deploy. If the app ships first, the add is accepted by the application and then rejected by the old database policy, turning today's clean "Recipe not found" into a "Failed to add recipe to cookbook" server error. **This migration has not been applied to any Supabase project — `020` is still the highest applied migration.**
- See [Cookbooks API](docs/api/cookbooks.md) and `docs/RELEASE_NOTES_REW-100.md` for what was and wasn't verified, and `database/README.md` for the policy change and its rollback.
...
- **Own-or-Published Recipe Visibility**: A user can add any of their own recipes (draft or published) to a meal plan, and can also add another user's *published* recipe — mirroring the same visibility rule already used by Recipe Likes. A user cannot add another user's draft/unpublished recipe; this is enforced at the database (RLS) level, not just in the UI. This used to be the difference between meal plans and cookbooks; as of REW-100 (migration `021`) cookbook membership follows the same rule, and migration `012` was the precedent it copied.
...
- CSRF is enforced globally for unsafe non-multipart requests, but the global wrapper exempts *every* multipart POST rather than only the Multer routes that need it. Newer state-changing routes therefore re-apply `csrfProtection` explicitly at the route level — `POST /recipes/:id/clone`, `POST /recipes/:id/visibility`, `POST /recipes/:id/delete` (REW-101, merged in PR #62), `POST /cookbooks/:id/delete` and `POST /meal-plans/:id/delete` (REW-105), and every `/api/cookbooks*` mutation. On all of these except `/:id/clone` the CSRF check runs ahead of the route's rate limiter, so a forged request cannot burn the victim's quota (the recipe delete route has no per-route limiter at all; the two REW-105 delete routes run `requireAuth` → `csrfProtection` → limiter → handler); the clone route runs `addRecipeLimiter` first, an ordering nit tracked under REW-99. Those three delete routes were the destructive ones with no route-level check — a forged cross-site multipart POST reached their handlers, because each reads no body fields — and REW-101's audit of every mutating route is recorded in the [API docs](docs/api/README.md#route-level-csrf-audit-rew-101). The central fix is tracked as [REW-99](https://wanderingnerds.atlassian.net/browse/REW-99).
- Row Level Security is the enforcement boundary for recipe collections, and REW-100 deliberately relaxed one policy: the `cookbook_recipes` INSERT policy now admits the caller's own recipe at any status **or** any user's Public recipe, matching what `meal_plan_recipes` has allowed since migration `012`. The cookbook-ownership half of that policy did not move, and another user's Private recipe is still rejected by Postgres. Two consequences to know before editing those routes: migration `010`'s header claim that cookbook membership is own-recipes-only "even if application code were buggy" is now historical, and the ownership filters on the two form-based add routes in `cookbookRoutes.js` are the **only** thing keeping those routes narrow — RLS no longer backs them up. See `database/README.md` and `docs/api/cookbooks.md`.
