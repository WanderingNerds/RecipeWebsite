...
- CSRF is enforced globally for unsafe non-multipart requests, but the global wrapper exempts *every* multipart POST rather than only the Multer routes that need it. Newer state-changing routes therefore re-apply `csrfProtection` explicitly at the route level — `POST /recipes/:id/clone`, `POST /recipes/:id/visibility`, `POST /recipes/:id/delete` (REW-101, merged in PR #62), `POST /cookbooks/:id/delete` and `POST /meal-plans/:id/delete` (REW-105, merged in PR #63), `POST /meal-plans/:id/recipes/:recipeId/remove`, `POST /cookbooks/:id/recipes/:recipeId`, `POST /api/meal-plans/:id/recipes/:recipeId` and `POST /api/likes/:recipeId` (REW-102), and every `/api/cookbooks*` mutation. On all of these except `/:id/clone` the CSRF check runs ahead of the route's rate limiter, so a forged request cannot burn the victim's quota (the recipe delete route has no per-route limiter at all; the rest run `requireAuth`/`requireApiAuth` → `csrfProtection` → limiter → handler); the clone route runs `addRecipeLimiter` first, an ordering nit tracked under REW-99. The four delete/remove routes were the destructive ones with no route-level check — a forged cross-site multipart POST reached their handlers, because each reads no body fields. The audit of **all 43 mutating routes**, completed under REW-102, is recorded in the [API docs](docs/api/README.md#route-level-csrf-audit-rew-101--rew-105--rew-102), along with the three session-lifecycle routes that remain forgeable ([REW-106](https://wanderingnerds.atlassian.net/browse/REW-106)) and a tripwire warning for three routes that are safe today only because a property access on an unparsed body throws. The central fix to the wrapper is tracked as [REW-99](https://wanderingnerds.atlassian.net/browse/REW-99).
