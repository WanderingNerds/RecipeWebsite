...
Deployment requires `RESEND_API_KEY`, `ASSIGNMENT_EMAIL_FROM`, and canonical `APP_URL`, plus uniquely active recognized profiles. Stored names may be `Andrew` or `Andrew Carroll`, and `Victoria` or `Victoria Johnson`; the UI labels remain Andrew and Victoria. No database migration or package dependency was added.

Final review is approved. The full Node suite passes 202/202 and `git diff --check` passes. Live Resend and browser acceptance remains pending because provider credentials and safe fixtures were unavailable; see [the QA record](qa/rew-78-admin-feedback-assignment-email.md).
