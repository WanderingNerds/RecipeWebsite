...
- **Check off = strike through, then leave off the printout** (`initializeGroceryListChecklist` in `public/js/meal-plans.js`): checking an item's box toggles a `.grocery-item--checked` class rather than deleting anything from the DOM, so it's struck through and muted on screen but still there to uncheck. The print stylesheet drops any `.grocery-item--checked` element entirely, so only what's still needed comes out of the printer. Unpersisted like everything else on this page — a refresh clears every checkbox.
- **Printing is deliberately condensed:** the print stylesheet lays categories out in two columns and uses smaller type than the on-screen view. The one hard rule that mattered here: `.grocery-category` must **not** use `break-inside: avoid` — that forces an entire category onto one page/column, and one long "Produce" list not fitting the remaining space is what used to blow the printout out to 4 pages. `break-inside: avoid-column` (category can't split across the two print columns, but can still turn a page) plus `break-after: avoid` on each heading (keeps a heading with its first item) gets a condensed, sane page count instead.
- **Small/Large print size toggle:** a radio pair in the page header sets a `.grocery-list-page--font-large` class before printing. Every rule for it lives inside `@media print`, so choosing "Large" has zero effect on the on-screen page — it only trades print page count for legibility. "Small" (the condensed sizes above) is the default.
- **Print-only brand line:** printing hides the navbar (the only place the "Potluck" name normally appears), so a `.grocery-list-brand` element carries it onto the printed page instead. Hidden on screen (`display: none`) since the navbar already covers that there; shown only inside `@media print`.
...
| Date | Change |
|------|--------|
| 2026-09-14 | REW-26 follow-up: fixed a shipped bug where every grocery list threw (undefined `pushVerbatim`/`item.volumeMl` mismatch); checking an item off now strikes it through and drops it from the printout; condensed printing to a two-column, smaller-type layout (and fixed the `break-inside: avoid` bug that forced a whole category onto one page); added a Small/Large print-size toggle and a print-only "Potluck" brand line. |
