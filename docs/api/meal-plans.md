...
**Middleware:** `requireAuth`, then route-level `csrfProtection`, then the existing `mealPlanLimiter` (30 requests/minute per user — no new limiter was added), in that order. The route-level `csrfProtection` is re-applied (REW-105) because the global `csrfProtectionExceptMultipart` wrapper skips token validation for any `multipart/form-data` body and this handler reads no body fields, so a forged cross-site multipart POST would otherwise reach it and delete the victim's own meal plan. CSRF runs *ahead of* the limiter so a forged request cannot burn the victim's 30/minute quota. The delete form posts `application/x-www-form-urlencoded` with a hidden `_csrf`, so the fix is transparent to it; a multipart POST is rejected with 403 even if it carries a valid `_csrf` part, because the route has no Multer stage. Pinned by `src/routes/mealPlanDeleteRoutes.test.js`.
...
As of REW-89 the form that posts here lives inside the shared card partial rather than in `views/meal-plans/view.ejs`, and is rendered only when the caller passes a `mealPlanId` local. That move changed nothing server-side: the route keeps both UUID guards, its plan-ownership check, and its `_csrf` hidden field. Unlike the cookbook equivalent it sends **no** `returnTo` field, because this route always redirects to `/meal-plans/:id`.

**Middleware:** `requireAuth`, then route-level `csrfProtection`, then the existing `mealPlanLimiter` (30 requests/minute per user — no new limiter was added), in that order. The route-level `csrfProtection` was added in REW-102 and is the reason this route matters more than its siblings: the global `csrfProtectionExceptMultipart` wrapper skips token validation for any `multipart/form-data` body, and this handler reads **no** body fields at all — unlike `POST /cookbooks/:id/recipes/:recipeId/remove`, which reads `req.body.returnTo` and therefore throws on an unparsed body — so a forged cross-site multipart POST used to reach it and delete the victim's `meal_plan_recipes` row. The REW-102 audit rated it the one **live destructive** CSRF hole left after REW-101 and REW-105. CSRF runs *ahead of* the limiter so a forged request cannot burn the victim's 30/minute quota. The card's Remove form posts `application/x-www-form-urlencoded` with a hidden `_csrf`, so the fix is transparent to it; a multipart POST is rejected with 403 even if it carries a valid `_csrf` part, because the route has no Multer stage. Pinned by `src/routes/mealPlanRecipeRemoveRoutes.test.js`.
...
`POST /api/meal-plans/:id/recipes/:recipeId` additionally runs route-level `csrfProtection` between `requireApiAuth` and the limiter (REW-102), matching its already-protected cookbook twin `POST /api/cookbooks/:id/recipes/:recipeId`. The only caller is `fetch` from `public/js/meal-plans.js`, and `public/js/main.js` patches `window.fetch` to attach `x-csrf-token` from `<meta name="csrf-token">` to every same-origin state-changing request — which the CSRF middleware accepts as an alternative to `req.body._csrf` — so the modal needed no client change. `POST /api/meal-plans` was left as it is: it validates its body before any write, so an unparsed body fails `validateMealPlanTitle`. `DELETE /api/meal-plans/:id/recipes/:recipeId` was also left as it is: HTML forms emit only GET/POST, and a cross-origin `fetch` with method DELETE is preflighted and refused by the CORS origin allow-list. Pinned by `src/routes/mealPlanApiRoutes.test.js`.
...
- **CSRF:** CSRF protection **is enforced** globally via `csrfProtectionExceptMultipart` in `src/app.js`; `POST` is not in `ignoredMethods` and `res.locals.csrfToken` is populated for every render. Every meal plan form, including the REW-69 visibility form, carries a real `_csrf` token and is rejected without one. (An earlier revision of this page stated CSRF was disabled repo-wide — that was correct when REW-63 shipped and is no longer true. Corrected 2026-09-14.) **Three routes on this surface additionally re-apply `csrfProtection` at the route level, ahead of their limiter:** `POST /meal-plans/:id/delete` (REW-105), `POST /meal-plans/:id/recipes/:recipeId/remove` and `POST /api/meal-plans/:id/recipes/:recipeId` (both REW-102). In each case the global wrapper skips multipart bodies and the handler reads no body fields, so each was forgeable cross-site until the route-level check was added. Placing CSRF before the limiter also means forged requests consume none of the victim's quota. **Still open on this surface:** `POST /meal-plans/:id/visibility` is safe only because `normalizeMealPlanVisibility` reads a property of an `undefined` body and throws — with an empty object it would fail closed to Private and silently un-share the plan. That is recorded as the REW-99 tripwire in the [route-level CSRF audit](README.md#rew-99-regression-tripwire-read-this-before-touching-the-wrapper) and must be fixed in the same change that touches the wrapper.
...
| Date | Change |
|------|--------|
| 2026-09-20 | REW-102: `POST /meal-plans/:id/recipes/:recipeId/remove` now runs `requireAuth` → route-level `csrfProtection` → `mealPlanLimiter` → handler, and `POST /api/meal-plans/:id/recipes/:recipeId` runs `requireApiAuth` → `csrfProtection` → `mealPlanApiLimiter` → handler. The remove route was the one **live destructive** multipart-CSRF hole left after REW-101/REW-105 — it reads no body fields, so a forged cross-site multipart POST deleted a membership row. No handler, validator, view, limiter or schema change; pinned by `src/routes/mealPlanRecipeRemoveRoutes.test.js` and `src/routes/mealPlanApiRoutes.test.js`. QA stage skipped — manual verification outstanding. |
| 2026-09-20 | REW-105: `POST /meal-plans/:id/delete` now runs `requireAuth` → route-level `csrfProtection` → `mealPlanLimiter` → handler, closing the multipart CSRF bypass on a destructive route (the global wrapper skips multipart bodies and this handler reads none). CSRF is placed ahead of the limiter so forged requests burn no quota. No handler, ownership-filter, view, or schema change; pinned by `src/routes/mealPlanDeleteRoutes.test.js`. |
