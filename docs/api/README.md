...
| POST | `/api/cookbooks/:id/recipes/:recipeId` | JSON: add a recipe to an **owned** cookbook — the caller's own recipe at any status, or anyone's published recipe (REW-100) — duplicate-safe |
| DELETE | `/api/cookbooks/:id/recipes/:recipeId` | JSON: remove the membership row (never the recipe) |

`src/routes/cookbookApiRoutes.js`, mounted at `/api/cookbooks`. Every route requires auth and returns JSON `401` if unauthenticated, consistent with `/api/likes*` and `/api/meal-plans*`. Mutations run `requireApiAuth` → `csrfProtection` → a 30-per-minute-per-user limiter; `GET /` carries no CSRF check by design, since it changes nothing. Authorization on the add is **cookbook-ownership + own-or-published** since **REW-100**: the cookbook must be the caller's (an explicit `.eq("user_id", req.user.id)` filter alongside RLS), while the recipe may be the caller's own at any status **or** any user's `status = 'published'` recipe. That rule is enforced twice and independently — as an explicit predicate in the handler, and at the database layer by migration `021`'s widened `cookbook_recipes` INSERT policy (the same shape `/api/meal-plans*` has had since migration `012`). Another user's **draft** recipe is still rejected at both layers, and every recipe reject returns the same generic `404 {"error":"Recipe not found"}`. The form-based `POST /cookbooks/:id/recipes/:recipeId` stays own-recipes-only. **Deploy-ordering gate: apply `021` before or together with the application deploy** — app-first turns that clean `404` into `500 {"error":"Failed to add recipe to cookbook"}`, since the old policy still rejects the insert. **`021` has not been applied to any Supabase project; `020` remains the highest applied migration, and QA did not run for REW-100.** See [My Recipes Recipe Card](my-recipes-card.md) and [Cookbooks API](cookbooks.md). **Branch-only: implemented and reviewed on `REW-86-standardize-my-recipes-card`, QA not run, not yet merged.**
...
- [Cookbooks API](cookbooks.md) - `/cookbooks*` endpoints, RLS-enforced privacy, the recipe view "Save to Cookbook(s)" integration (REW-62), Private/Public cookbook sharing via `POST /cookbooks/:id/visibility` and the public `GET /c/:id` (REW-19), and the own-or-published recipe rule on the JSON API add with its migration-`021` deploy-ordering gate (REW-100)
...
Every `router.post` / `router.delete` in `src/routes/` was audited on `main` at `8f6eb30` for [REW-101](https://wanderingnerds.atlassian.net/browse/REW-101) / [REW-102](https://wanderingnerds.atlassian.net/browse/REW-102) (there are no `put`/`patch` routes and no mutating routes outside the routers; 43 mutating routes in total). Two facts drive the *forgeable today* column:

- Express 5 leaves `req.body` **undefined** when no body parser matched, so a cross-site `multipart/form-data` form POST reaches a handler with no body. If the handler never reads `req.body` before mutating, the mutation runs. If it reads `req.body.x` first it throws a `TypeError` and nothing happens — incidental, not a control.
- `DELETE`-verb routes are not reachable by the HTML-form vector at all: forms only send GET and POST, and a cross-origin `fetch` DELETE is preflighted and refused by the `cors` origin configuration.
...
| `POST /auth/logout`, `POST /admin/logout` | (none) | No | Yes — logs the victim out | Low (logout CSRF). REW-99. |
| `POST /cookbooks/:id/recipes/:recipeId/remove`; `POST /cookbooks`, `/cookbooks/:id/update`, `/cookbooks/:id/visibility`, `/cookbooks/:id/add-recipes`; `POST /meal-plans`, `/meal-plans/:id/update`, `/meal-plans/:id/visibility`, `/meal-plans/:id/add-recipes`; `POST /api/meal-plans`; `POST /api/tags`; `POST /recipes/import/save`; `POST /help-feedback`; `POST /admin/feedback/:id`, `/admin/feedback/:id/comments`; `POST /admin/login` | requireAuth / requireApiAuth / requireAdmin (+ limiter) | Yes | No — the body read precedes the mutation and throws on an undefined body | Incidental, not a control. REW-99. |
| `POST /auth/login`, `/register`, `/forgot-password`, `/resend-confirmation`, `/reset-password` | redirectIfAuthenticated / none | Yes | No | Login-CSRF class in principle. REW-99. |
| `POST /auth/reset-password/session` | (none) | Tolerates an undefined body | Expected to fail validation on empty input — **not verified** in this audit | REW-99; verify when that ticket is worked. |
